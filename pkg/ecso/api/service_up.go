...
func (api *api) ServiceUp(project *ecso.Project, env *ecso.Environment, service *ecso.Service) error {
	...
	if err := api.deployServiceStack(reg, project, env, service); err != nil {
		return err
	}
...
func (api *api) deployServiceStack(reg *ecso.AWSClientRegistry, project *ecso.Project, env *ecso.Environment, service *ecso.Service) error {
	var (
		cfg       = api.cfg
		log       = cfg.Logger
...
func getServiceStackParameters(cfn services.CloudFormationService, project *ecso.Project, env *ecso.Environment, service *ecso.Service) (map[string]string, error) {
